...
gcDBCore cm ot k gc repeat = do
  ...
  let (effRows, gcMarker) = foldl (\(effAcc,gcAcc) (sid,sqn,time,deps,val,txnid,shimid,rseqno) ->
        case txnid of
          Just _ -> (effAcc, gcAcc) -- XXX KC; Handle transactions
          Nothing ->
            case val of
              EffectVal bs -> ((sid,sqn,time,deps,bs,shimid,rseqno):effAcc, gcAcc)
  ...
  let (effSet, depsMap) =
        foldl (\(setAcc, mapAcc) (sid, sqn, time, deps, eff,shimid,rseqno) ->
                  (S.insert (Addr sid sqn, eff, time, shimid, rseqno) setAcc,
  ...
  let (addrList, effList, timeList, shimidList, rseqnoList) = unzip5 (S.toList filteredSet)
  let gcedEffList = gc effList
  -- Allocate new gc address
  let gcAddr = Addr gcSid 1
  -- Pick the last of the shim ids for the summarized op
  let shimId = last shimidList
  -- Pick the max of the rseqno's for the summarized op
  let rseqno = maximum rseqnoList
  let (outRows, count) =
        foldl (\(acc, idx) eff ->
                  ((gcSid, idx, S.singleton gcAddr, EffectVal eff, Nothing, shimId, rseqno):acc, idx+1))
              ([],2) gcedEffList
  runCas (cm^.pool) $ do
    -- Insert new effects
    ...
    cqlInsert ot ALL k (gcSid, 1, newDeps, GCMarker gcTime, Nothing, shimId, rseqno)
...
filterUnresolved :: CursorAtKey                   -- Key Cursor
                 ...
                 -> S.Set (Addr, Effect, UTCTime, ShimID, SeqNo) -- Unfiltered input
                 -> S.Set (Addr, Effect, UTCTime, ShimID, SeqNo) -- Filtered result
